...
public class AccountDao extends DBConnectionCloser{
    ...
    public void createAccount(String username, String password, InputStream image, int seller, int admin) {
        String query = "INSERT INTO account (account_name, account_password, account_image, account_is_seller, account_is_admin) VALUES (?, ?, ?, ?, ?)";
        try {
            Class.forName("com.mysql.jdbc.Driver");
            connection = new Database().getConnection();
            preparedStatement = connection.prepareStatement(query);
